  ...
  if (emoji.id === verify.split(':')[2].slice(0, -1)) {
    ...
    if (!member.roles.cache.has(verificationRole)) {
      try {
        ...
        return client.sendSystemErrorMessage(member.guild, 'Weryfikacja',
          stripIndent`Unable to assign verification role,` +
          'proszę sprawdzić hierarchię ról i upewnić się, że mam uprawnienia Manage Roles (Zarządzaj rolami)'
          , err.message);
      }
    }
  }
